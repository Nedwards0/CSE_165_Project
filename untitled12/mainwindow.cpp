...
void MainWidget::createSeries()
{
    ...
    QFile covidData("‪C:\\Users\\SWE\\OneDrive\\Documents\\GitHub\\CSE_165_Project\\Book1.xlsx");
    if(!covidData.open(QIODevice::ReadOnly | QIODevice::Text))
        return;

    BoxDataReader dataReader(&covidData);
    while(!dataReader.atEnd()) {
        //QBarSeries *set = dataReader.ReadBox();
        QBarSet *set = dataReader.ReadBox();
        if (set)
            m_series->append(set);
    }
